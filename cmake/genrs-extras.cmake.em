...
macro(_generate_msg_rs ARG_PKG ARG_MSG ARG_IFLAGS ARG_MSG_DEPS ARG_GEN_OUTPUT_DIR)
  ...
  set(MSG_GENERATED_NAME ${MSG_SHORT_NAME}.rs)
...
macro(_generate_module_rs)
  # the macros, they do nothing
endmacro()

set(genrs_INSTALL_DIR include)

macro(genrs_append_include_dirs)
  if(NOT genrs_APPENDED_INCLUDE_DIRS)
    # make sure we can find generated messages and that they overlay all other includes
    include_directories(BEFORE ${CATKIN_DEVEL_PREFIX}/${genrs_INSTALL_DIR})
    # pass the include directory to catkin_package()
    list(APPEND ${PROJECT_NAME}_INCLUDE_DIRS ${CATKIN_DEVEL_PREFIX}/${genrs_INSTALL_DIR})
    set(genrs_APPENDED_INCLUDE_DIRS TRUE)
  endif()
endmacro()
